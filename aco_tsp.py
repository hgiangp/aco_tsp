...
class SolveTSPUsingACO:
    ...
    class Ant:
        ...
        def _select_node(self):
            ...
            for unvisited_node in unvisited_nodes:
                # if (self.tour[-1] == 15 and unvisited_node == 14): 
                    # print(self.edges)
        ...
        for i in range(self.num_nodes):
            for j in range(i + 1, self.num_nodes):
                ...
                if edge_weight < 0.0001: 
                    edge_weight = 0.0001
                self.edges[i][j] = self.edges[j][i] = self.Edge(i, j, edge_weight, initial_pheromone)
    ...
    def _max_min(self):
        for step in range(self.steps):
            ...
            self.iter_x.append(step)
            self.iter_y.append(self.global_best_distance)
    ...
    def plot(self, line_width=1, point_radius=math.sqrt(2.0), annotation_size=8, dpi=120, save=True, name=None):
        ...
        plt.scatter(x, y, c='r', marker='o', s=math.pi * (point_radius ** 2.0))
        plt.xlabel(f'Total mileage of the tour: {round(self.global_best_distance, 2)}')
        plt.title('ACO')
        ...
        plt.savefig(f'{name}_conversion.png', dpi=dpi)   
        # plt.show()
        # plt.gcf().clear()
        plt.close()
...
if __name__ == '__main__':
    _colony_size = 20
    _steps = 250
    input_path = "./tsp100.txt"
    _nodes = util.read_data(input_path)
    # _nodes = [(random.uniform(-400, 400), random.uniform(-400, 400)) for _ in range(0, 1000)]
    # print(_nodes)
